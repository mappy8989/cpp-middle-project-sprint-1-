...
public:
    ...
    bool EncryptFile(std::iostream &inStream, std::iostream &outStream, std::string_view password) {
        ...
        EVP_CipherInit_ex(cipher_ctx_.get(), params.cipher, nullptr, params.key.data(), params.iv.data(),
                          params.encrypt);
        ...
        while (in_shift < in_size) {
            int chunk_size = (in_size - in_shift) > MAX_CHUNK_SIZE ? MAX_CHUNK_SIZE : (in_size - in_shift);
            // Обрабатываем оставшиеся символы
            EVP_CipherUpdate(cipher_ctx_.get(), out_buf, &out_Len, &in_vec.data()[in_shift], chunk_size);
    ...
    bool DecryptFile(std::iostream &inStream, std::iostream &outStream, std::string_view password) {
        auto params = CreateChiperParamsFromPassword(password);
        ...
        std::vector<unsigned char> plaintext(in_size +
        ...
        if (EVP_CipherUpdate(cipher_ctx_.get(), plaintext.data(), &len, ciphertext.data(), ciphertext.size()) != 1) {
            return false;
        }

        plaintext_len = len;
        if (EVP_CipherFinal_ex(cipher_ctx_.get(), plaintext.data() + plaintext_len, &len) != 1) {
            return false;
        }
        plaintext_len += len;

        outStream.write(reinterpret_cast<const char *>(plaintext.data()), plaintext_len);
